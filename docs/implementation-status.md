...
The Docker Compose adapter now has bounded lifecycle methods for validation,
planning, activation, status, termination, reset, and evidence export.
...
9. The Edge decision ID is consumed atomically and cannot be reused.

The current reference package intentionally fails gate 7 because its OCI
provenance/SBOM/digests remain bootstrap placeholders. Therefore current main
does **not** provide an activatable reference decoy.
...
- A consumed decision stays consumed even if the later runtime operation fails;
  retry requires a new Edge decision.
...
- `Azazel-Deception#2` — lifecycle adapter: code integrated; live lab validation still open
