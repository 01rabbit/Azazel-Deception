...
- [x] A real container completes the full activation/evidence/termination/reset software lifecycle end-to-end.
      *Evidence: green run [35455625101](https://github.com/01rabbit/Azazel-Deception/actions/runs/35455625101) of `.github/workflows/executed-evidence.yml` — the `docker-lifecycle` job ran `tests/test_docker_integration.py::test_gated_live_lifecycle_enforces_isolation_and_preserves_evidence` with `AZAZEL_DECEPTION_DOCKER_TESTS=1`, and the `virtual-lab` job drove `make virtual-lab` against a real container with the real `GitHubAttestationPackageVerifier` and archived `artifacts/lab/virtual-phase1-lab.json` as a workflow artifact. Both jobs assert the evidence actually ran: `scripts/ci/assert_executed.py` rejects a green-but-skipped pytest result, and `scripts/ci/assert_lab_report.py` rejects a report that does not show activation, termination, reset, an intact evidence chain and consumed one-shot decisions. This is a software claim only, not a physical/HIL isolation proof. `LIVE_GATES` id `software_real_container_lifecycle_executed`.*
- [x] Real container termination/reset after **attacker modification** is demonstrated, with the evidence chain finalized and verified.
      *Evidence: green run [35455625101](https://github.com/01rabbit/Azazel-Deception/actions/runs/35455625101), `docker-lifecycle` job — `tests/test_docker_integration.py::test_attacker_modified_state_is_destroyed_by_termination` (attacker state written into the running decoy, read-only rootfs refusing persistence, termination, and a fresh activation carrying no attacker-modified state), with `::test_container_crash_is_recovered_by_kill_switch` and `::test_termination_failure_fails_closed_and_kill_switch_recovers` in the same run. `scripts/ci/assert_executed.py --min-tests 8` makes the job fail if any of the eight skips, so the green badge cannot stand for an unexecuted suite. `LIVE_GATES` id `software_attacker_modified_reset_executed`.*
...
- [x] A full networked, mutually-authenticated Edge-to-AZ-06 heartbeat and automatic state-reconciliation loop is proven end-to-end.
      *Both sides were already implemented — the AZ-06 `heartbeat`/`reconcile` actions (checked above) and the Edge-side `HeartbeatLoop` in `Azazel-Edge/py/azazel_edge/deception_shadow_client.py`, which polls on an interval, tracks consecutive failures, reconciles against the Edge active set and fires an `on_divergence` hook. What was missing was an executed run: the named proof, `Azazel-Edge/tests/test_deception_shadow_heartbeat_e2e.py`, opens with `pytest.importorskip("azazel_deception")` and Edge CI (`.github/workflows/ci.yml`) installs only `requirements/runtime.txt` and `requirements/fabric.txt` — never `azazel-deception` — so that test skipped in every recorded Edge run. *Evidence: green run [35455625101](https://github.com/01rabbit/Azazel-Deception/actions/runs/35455625101), `cross-repo-heartbeat-e2e` job — it checks out Azazel-Edge alongside AZ-06, puts `edge/py` on `PYTHONPATH`, runs `Azazel-Edge/tests/test_deception_shadow_heartbeat_e2e.py` (3 tests: authenticated heartbeat on an interval, automatic reconciliation against Edge's authoritative active set with a divergence report, and fail-closed liveness when the node disappears), and `scripts/ci/assert_executed.py --min-tests 3` fails the job if the suite `importorskip`s away. It runs in this repository, not Edge's, because installing `azazel-deception` into Edge CI would invert the dependency direction — an AZ-06 regression would redden Edge's baseline, and Edge must stay fully functional with no AZ-06 present. Zero container start, zero enforcement, descriptive only. `LIVE_GATES` id `software_networked_heartbeat_e2e_executed`.*
