...
`LIVE_GATES` does not yet carry a gate id for every `[ ]` item below; the gaps
are called out on the affected lines.
...
- [ ] Equivalent end-to-end activation/evidence/termination/reset semantics are demonstrated on both architectures.
      *Missing: the only real-container lifecycle proof is the opt-in `tests/test_docker_integration.py`, which no workflow executes, so neither architecture has a recorded end-to-end lifecycle and the two are not compared. `LIVE_GATES` id `portability_equivalent_e2e_on_arm64_and_amd64`.*
